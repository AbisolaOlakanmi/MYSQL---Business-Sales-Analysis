-- create the database
DROP DATABASE IF EXISTS BDAT1000_AbisolaOlakanmi;
CREATE DATABASE BDAT1000_AbisolaOlakanmi;

-- select the database
Use BDAT1000_AbisolaOlakanmi;
...
-- creating new table to have all null stored

CREATE TABLE New_Vendors as (
...
and vendor_zip_code is not null
and vendor_phone is not null
and vendor_contact_last_name is not null
and vendor_contact_first_name is not null
and default_terms_id is not null
and default_account_number is not null
 );
 
 select * from New_Vendors;
   
-- creating new invoice table to have all non null stored
CREATE TABLE New_Invoice as (
...
-- creating new terms table to have all non null values stored

CREATE TABLE New_terms as (
SELECT * FROM terms WHERE 
	terms_id IS NOT NULL
	and terms_description IS NOT NULL
    and terms_due_days IS NOT NULL);
    
  select * from New_terms;      
  
  
-- creating new general_ledgeracct  table to have all non null values stored

CREATE TABLE New_generalledgeracct as (
SELECT * FROM general_ledger_accounts WHERE 
	account_number IS NOT NULL
	and account_description IS NOT NULL
);
    
-- G1 creating a new table to join terms and invoice

create table terms_invoices as (
SELECT new_invoice.*, new_terms.terms_due_days, new_terms.terms_description
FROM 
    new_invoice
INNER JOIN 
    new_terms ON new_invoice.terms_id = new_terms.terms_id
WHERE new_terms.terms_due_days >= 30);

  select * from terms_invoices;   

/*G2 creating a new table to*/

CREATE TABLE invoices_generalledger_terms AS (

SELECT 
    new_vendors.*,
    new_generalledgeracct.*,
    new_terms.*
FROM 
    new_vendors
JOIN 
    new_generalledgeracct ON new_vendors.default_account_number = new_generalledgeracct.account_number
JOIN 
    new_terms ON new_vendors.default_terms_id= new_terms.terms_id
WHERE 
	new_terms.terms_due_days >= 30);

  select * from invoices_generalledger_terms;   


DROP TABLE invoices_generalledger_terms;
